  ...
  implements GGButtonListener
{
  ...
  // define 2 new attributes add for task
  private final int numDice;
//  private AllDies allDies =;
  ...
  NavigationPane(Properties properties)
  {
    ...
    this.numDice =  //Number of six-sided dice
            (properties.getProperty("dice.count") == null)
                    ? 1  // default
                    : Integer.parseInt(properties.getProperty("dice.count"));
//    this.allDies = new AllDies(this, numDice);

    System.out.println("numberOfDice = " + numDice);
  ...
  private int getDieValue() {
    ...
    int currentRound = (nbRolls / gp.getNumberOfPlayers()) * numDice + order;
...
//  the output message for task5
  ...
  void prepareRoll(int currentIndex)
  {
    ...
    else
    {
      ...
      // check 2 puppet collision
      if (!gp.getPuppet().isTravel_path()) {
        checkCollison(currentIndex);
      }
  ...
  // check 2 puppet collision, if they are collision, current puppet will move back
  private void checkCollison(int currentIndex){
    int currPlayer = gp.getCurrentPuppetIndex();
    int player = 0;
    for (Puppet p : gp.getAllPuppets()) {
      if (player != currPlayer && p.getCellIndex() == currentIndex) {
        p.setBack(true);
        p.go(-1);
      }
      player++;
  ...
  void startMoving(int nb)
  {
    ...
    // tell puppet to travel this number of steps
    gp.getPuppet().go(nb);
  }
  ...
  // if dice count reached to max, then stop rolling
  void prepareAfterRoll() {
    if (dieList.size() == numDice
        || gp.getAllPuppets().get(gp.getCurrentPuppetIndex()).isAuto()) {
      handBtn.setEnabled(false);
    } else {
      handBtn.setEnabled(true);
    }
  ...
  public void buttonClicked(GGButton btn)
  {
    ...
    // stop rolling if dice count is reached
    prepareAfterRoll();
  }
  ...
  private void roll(int rollNumber)
  {
    ...
    addNewDie(nb);
    // get the latest die and add actor
    addActor(dieList.get(dieList.size()-1), dieBoardLocation);
  }
  ...
  public void toggleButton(boolean changed) {
    // if toggleButton was changed then reverse all Connections
    if (!changed && toggleCheck.isChecked() ) {
      gp.allConnectionsReverse();
    }
    else if (changed && !toggleCheck.isChecked() ) {
      gp.allConnectionsReverse();
    }
      this.toggleCheck.setChecked(changed);
  }

  //add new die to die list, and add the sum up
  public void addNewDie(int nb) {
    int size = dieList.size();
    Die die = new Die(size+1, nb, this);
    dieList.add(die);
    sumValue += nb;
    order++;
  }
  ...
  //check all dies were rolled, then start moving and clear dielist
  public void checkFinish(int index) {
    System.out.println("index: " + index);
    if (index == numDice) {
      startMoving(sumValue);
      this.sumValue = 0;
      this.order = 0;
      this.dieList.clear();
    }
